...
Let $N^*(t)$ denote the expected number of recurrent events by time $t$ and let $D^*$ denote the time of death. 
...
It is assumed that $C$ is independent of $Z$. Moreover, it is assumed that $N^*(\cdot)$ and $D^*$ are conditionally independent given $Z$.  
